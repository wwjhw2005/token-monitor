...
3. funnels output through `extractUsageFromTokscale()` in `src/shared/usage.js`, which is a defensive deep-walker over tokscale's JSON shape (it never assumes a fixed layout — that's why `tokenValue`/`detectClient` accept many key spellings);
   - Grok's newer `updates.jsonl` records include a stable per-turn input/output/cache breakdown that tokscale's cumulative-context parser can flatten into input-only usage. `src/shared/grokUsage.js` reconciles those sessions before extraction: complete new-format windows replace the tokscale rows, while mixed old/new windows preserve tokscale input and supplement only the categories proven by the new records. Keep this reconciliation ahead of `applyPeriodDelta()` so watch-triggered month/all-time updates remain exact, and reconcile graph history before the daily archive so both trend views and retained days use corrected totals;
