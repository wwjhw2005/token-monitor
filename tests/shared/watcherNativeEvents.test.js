  ...
  try {
    ...
    const seen = new Promise((resolve) => {
      watcher.on('all', (event, filePath) => {
        ...
        // macOS FSEvents can re-emit the watch root as addDir after ready. That is
        // not a content change — keep waiting for a real path under the root so
        // awaitWriteFinish'd writes are not asserted against an early root event.
        if (event === 'addDir' && path.resolve(filePath) === root) return;
        resolve();
      });
    });
    await act();
